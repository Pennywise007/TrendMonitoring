...
#include "helpers/SamplesHelper.h"
...
TEST_F(MonitoringTestClass, CheckTrendMonitoring)
{
    ...
    EXPECT_STREQ(monitoringChannel->c_str(), L"Прогибометр №1") << "Список каналов мониторинга отличается от эталонного";
    EXPECT_STREQ((++monitoringChannel)->c_str(), L"Прогибометр №2") << "Список каналов мониторинга отличается от эталонного";
    EXPECT_STREQ((++monitoringChannel)->c_str(), L"Прогибометр №3") << "Список каналов мониторинга отличается от эталонного";
    ...
    compareWithResourceFile(ext::get_service<TestHelper>().getConfigFilePath().c_str(),
...
void MonitoringTestClass::ExpectAddTask(const size_t channelIndex,
                                        std::optional<std::wstring> currentChannelName,
                                        std::optional<MonitoringInterval> currentInterval)
{
    ...
    EXPECT_CALL(*m_monitoringServiceMock.get(), AddTaskList(_, Matcher(IMonitoringTasksService::TaskPriority::eNormal))).
        WillOnce(Invoke([taskId, chanName = std::move(currentChannelName), interval = std::move(currentInterval)]
        (const std::list<TaskParameters::Ptr>& listTaskParams, const IMonitoringTasksService::TaskPriority /*priority*/)
        {
            ...
            if (chanName.has_value())
                EXPECT_STREQ(listTaskParams.front()->channelName.c_str(), chanName->c_str()) << "Имя канала не совпало";
            if (interval.has_value())
                EXPECT_EQ(listTaskParams.front()->endTime - listTaskParams.front()->startTime, monitoring_interval_to_timespan(interval.value())) << L"Интервал запроса данных не совпал";
...
//----------------------------------------------------------------------------//
void MonitoringTestClass::checkModelAndRealChannelsData(const std::wstring& testDescr)
{
    auto reportText = [&testDescr](const size_t index, const std::wstring& extraText)
    {
        return std::string_swprintf(L"%s: Различаются данные по каналу №%u, различаются %s.", testDescr.c_str(), index, extraText.c_str());
    };
    ...
    for (auto&& [taskId, modelChannelData] : m_channelsData)
    {
        ...
        EXPECT_EQ(modelChannelData.bNotify, serviceChannelData.bNotify) << reportText(index, L"флаг включенности оповещений");
        EXPECT_STREQ(modelChannelData.channelName.c_str(), serviceChannelData.channelName.c_str()) << reportText(index, L"названия каналов");
        EXPECT_EQ(modelChannelData.monitoringInterval, serviceChannelData.monitoringInterval) << reportText(index, L"интервалы мониторинга");

        if (isfinite(modelChannelData.alarmingValue) && isfinite(serviceChannelData.alarmingValue))
            // если они не наны - сравниваем числа
            EXPECT_FLOAT_EQ(modelChannelData.alarmingValue, serviceChannelData.alarmingValue) << reportText(index, L"оповещательное значение");
        else
            // если кто-то нан то второй тоже должен быть наном
            EXPECT_EQ(isfinite(modelChannelData.alarmingValue), isfinite(serviceChannelData.alarmingValue)) << reportText(index, L"оповещательное значение");

        ++index;
    }

    EXPECT_EQ(m_monitoringService->GetNumberOfMonitoringChannels(), m_channelsData.size()) << (std::narrow(testDescr) + ": Различаются количество каналов.").c_str();
}

//----------------------------------------------------------------------------//
void MonitoringTestClass::testAddChannels(size_t count)
{
    const std::wstring defaultChannelName = *m_monitoringService->GetNamesOfAllChannels().begin();
    ...
    checkModelAndRealChannelsData(L"Добавление каналов");
}
...
void MonitoringTestClass::testSetParamsToChannels()
{
    ...
    // проверяем что у локального списка каналов данные совпадают
    checkModelAndRealChannelsData(L"Установка настроек");
}
...
void MonitoringTestClass::testChannelListManagement()
{
    ...
    // проверяем что у локального списка каналов данные совпадают
    checkModelAndRealChannelsData(L"Перемещение каналов в списке");
...
void MonitoringTestClass::testDelChannels()
{
    ...
    // проверяем что у локального списка каналов данные совпадают
    checkModelAndRealChannelsData(L"Удаление каналов из списка");
