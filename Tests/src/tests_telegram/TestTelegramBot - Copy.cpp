...
TestTelegramBot::TestTelegramBot()
    : ServiceProviderHolder(ext::get_service<ext::ServiceCollection>().BuildServiceProvider())
    , m_pUserList(GetInterface<telegram::users::TestTelegramUsersList>())
    , m_telegramThread(GetInterface<TestTelegramThread>())
{}
...
void TestTelegramBot::SetUp()
{
    ...
    m_testTelegramBot = std::make_unique<CTelegramBot>(m_serviceProvider, GetInterface<telegram::users::ITelegramUsersList>(), m_telegramThread);
    ...
    pTelegramThread->onSendMessage(
         ...
         const std::string& parseMode, bool disableNotification)
        {
            ...
            EXPECT_TRUE(msg == *pExpectedMessage) << "Пришло сообщение отличающееся от ожидаемого.\n\nСообщение:\n" <<
                CStringA(msg.c_str()) << "\n\nОжидалось:\n" << CStringA(pExpectedMessage->c_str());
    ...
    pTelegramThread->onSendMessageToChats(
         ...
         const std::string& parseMode, bool disableNotification)
        {
            ...
            EXPECT_TRUE(msg == *pExpectedMessageToRecipients) << "Пришло сообщение отличающееся от ожидаемого.\n\nСообщение:\n" <<
                CStringA(msg.c_str()) << "\n\nОжидалось:\n" << CStringA(pExpectedMessageToRecipients->c_str());
