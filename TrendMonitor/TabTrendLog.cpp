...
void CTabTrendLog::OnNewLogMessage(const std::shared_ptr<LogMessageData>& logMessageData)
{
    ...
    // делаем отступ справа на каждый перенос строки чтобы выровн€ть по горизонтали
    ...
    case LogMessageData::MessageType::eError:
        ...
        break;
    default:
        EXT_ASSERT(!"Ќеизвестный тип сообщени€");
        [[fallthrough]];
