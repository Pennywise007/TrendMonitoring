...
std::wstring ChannelDataGetter::GetSignalsDir()
{
    try
    {
        auto dirsService = ext::GetInterface<IDirService>(ext::get_service<ext::ServiceCollection>().BuildServiceProvider());
        return dirsService->GetZetSignalsDir();
    }
    catch (...)
    {
        throw std::runtime_error(std::narrow(ext::ManageExceptionText(L"Не удалось найти директорию с сигналами в реестре. "
            L"Ожидаемый путь в реестре: HKLM\\SOFTWARE\\ZETLAB\\Settings DirSignal").c_str()).c_str());
    }
}
...
std::wstring ChannelDataGetter::GetCompressedDir()
{
    ...
    {
        auto dirsService = ext::GetInterface<IDirService>(ext::get_service<ext::ServiceCollection>().BuildServiceProvider());
        return dirsService->GetZetCompressedDir();
    }
    catch (...)
    {
        throw std::runtime_error(std::narrow(ext::ManageExceptionText(L"Не удалось найти директорию с сигналами в реестре. "
            L"Ожидаемый путь в реестре: HKLM\\SOFTWARE\\ZETLAB\\Settings DirCompressed").c_str()).c_str());
    }
}
...
void ChannelDataGetter::getSourceChannelData(const CTime& timeStart, const CTime& timeEnd, std::vector<float>& data, bool removeNAN)
{
    ...
    if (tsDelta.GetTotalSeconds() <= 0)
        throw std::exception(std::narrow(L"Неверный временной интервал для загрузки исходных данных").c_str());
